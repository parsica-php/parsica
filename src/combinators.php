...
function either(Parser $first, Parser $second): Parser
{
    ...
    return Parser::make($label, static function (Stream $input) use ($second, $first, $label): ParseResult {
        ...
        $r1 = $first->run($input);
        if ($r1 instanceof Succeed) {
            return $r1;
        }
        $r2 = $second->run($input);

        if ($r2 instanceof Succeed) {
            return $r2;
        }
...
function atLeastOne(Parser $parser): Parser
{
    return Parser::make(
        ...
        static function (Stream $input) use ($parser) : ParseResult {
            ...
            while ($result instanceof Succeed) {
...
function notFollowedBy(Parser $parser): Parser
{
    ...
    $p = Parser::make($label, static function (Stream $input) use ($label, $parser): ParseResult {
        $result = $parser->run($input);
        return $result instanceof Succeed
...
function lookAhead(Parser $parser): Parser
{
    return Parser::make(
        ...
        static function (Stream $input) use ($parser): ParseResult {
            ...
            return $parseResult instanceof Succeed
